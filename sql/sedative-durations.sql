...
DROP MATERIALIZED VIEW IF EXISTS SEDATIVEDURATIONS;
CREATE MATERIALIZED VIEW SEDATIVEDURATIONS as
with io_cv as
(
  select
    icustay_id, charttime, itemid, stopped
    , case when itemid IN (30124, 30118, 30149, 30150, 30308, 30163, 30131) then rate else null end as rate
    , case when itemid IN (30124, 30118, 30149, 30150, 30308, 30163, 30131) then amount else null end as amount
  from mimiciii.inputevents_cv
  where itemid in
  (
    30124, 30118, 30149, 30150, 30308, 30163, 30131
  )
)
-- select only the ITEMIDs from the inputevents_mv table related to sedatives
, io_mv as
(
  select
...
(
  select v.*
    ...
    , case
        ...
        -- this captures patients who die/are discharged while on sedative
