...
def buildGraph():
	...
	def getCell():
		...
		cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=0.6)
		return cell

	# Forward direction cell
	lstmFwCell = tf.nn.rnn_cell.MultiRNNCell([getCell() for _ in range(nLayers)])
	# Backward direction cell
	lstmBwCell = tf.nn.rnn_cell.MultiRNNCell([getCell() for _ in range(nLayers)])
