...
def buildGraph(numFeatures=numFeatures, stateSizes=stateSizes):
    ...
    weight = tf.placeholder(tf.float32, [None], name='weight')  # [batchSize]
    ...
    score_ta = tf.TensorArray(dtype=tf.float32, size=num_steps, name='score_ta', clear_after_read=False)
    urine_output_placeholder = tf.placeholder(tf.float32, [None, None])  # [batchSize, num_steps]
    urine_output = tf.clip_by_value(urine_output_placeholder, 0, 3000)
    urine_output = urine_output / 3000
    urine_output_transpose = tf.transpose(urine_output, [1, 0])  # [num_steps, batchSize]
    urine_output_ta = tf.TensorArray(tf.float32, size=num_steps, name='urine_output_ta').unstack(urine_output_transpose)
    urine_output_predicted_ta = tf.TensorArray(dtype=tf.float32, size=num_steps, name='urine_output_predicted_ta', clear_after_read=False)
    ...
    feedback_place_holder = tf.get_variable('feedback_place_holder', [1, 32], dtype=tf.float32)
    ...
    b_regularizer = tf.get_variable('b_regularizer', [numFeatures], initializer=tf.constant_initializer(0.0),
                                    dtype=tf.float32)
    feedback_W1 = tf.get_variable('feedback_W1', [4 + 2 * numFeatures, 32],
                                  initializer=tf.contrib.layers.xavier_initializer(),
                                  dtype=tf.float32)
    feedback_b1 = tf.get_variable('feedback_b1', [32], initializer=tf.constant_initializer(0.0), dtype=tf.float32)
    feedback_W2 = tf.get_variable('feedback_W2', [32, 32], initializer=tf.contrib.layers.xavier_initializer(),
                                  dtype=tf.float32)
    feedback_b2 = tf.get_variable('feedback_b2', [32], initializer=tf.constant_initializer(0.0), dtype=tf.float32)

    def condition(unused_time, elements_finished, *_):
        return tf.logical_not(tf.reduce_all(elements_finished))

    def body(time, elements_finished, current_input, states, score_ta, y_predicted_ta, y_ta, x_predicted_ta, urine_output_predicted_ta):
        ...
        def body_without_feedback(current_input, states, y_predicted_ta, y_ta):
            ...
            urine_output_predicted = tf.nn.sigmoid(tf.matmul(last_output, W) + b)  #[batchSize, 1]
            urine_output_predicted = tf.clip_by_value(urine_output_predicted, 1e-10, 1.0 - 1e-10)
            urine_output_predicted = tf.reshape(urine_output_predicted, [batchSize])
            predicts = tf.cast((urine_output_predicted*3000/weight/6) <= 0.5, tf.float32)
            score = 0.5 - urine_output_predicted * 3000 / weight / 6
            score = tf.clip_by_value(score, -0.5, 0.5) * 8
            score = tf.sigmoid(score)
            x_predicted = tf.sigmoid(tf.matmul(last_output, W_regularizer) + b_regularizer)  # [batchSize, numFeatures]
            return predicts, urine_output_predicted, x_predicted, new_states, y_predicted_ta, y_ta, score

        def body_with_feedback(current_input, states, y_predicted_ta, y_ta, x_predicted_ta, urine_output_predicted_ta):
            new_states = [None] * len(states)
            last_output = current_input
            ...
            urine_output_feedback = urine_output_ta.read(time-6)
            urine_output_feedback = tf.expand_dims(urine_output_feedback, -1)
            urine_output_predicted_feedback = urine_output_predicted_ta.read(time-6)
            urine_output_predicted_feedback = tf.expand_dims(urine_output_predicted_feedback, -1)
            x_predicted_feedback = x_predicted_ta.read(time-1)
            feedback = tf.concat(
                [
                    y_predicted_feedback, y_feedback,
                    urine_output_predicted_feedback, urine_output_feedback,
                    x_predicted_feedback, current_input
                ],
                axis=1)  # [batchSize, 4+2*numFeatures]
        ...
        predicts, urine_output_predicted, x_predicted, new_states, y_predicted_ta, y_ta, score = tf.cond(time > 6,
        ...
        score_ta = score_ta.write(time, score)
        urine_output_predicted_ta = urine_output_predicted_ta.write(time, urine_output_predicted)
        ...
        return time, elements_finished, next_input, new_states, score_ta, y_predicted_ta, y_ta, x_predicted_ta, urine_output_predicted_ta
    ...
    returned = tf.while_loop(condition, body,
                             loop_vars=[time, elements_finished, first_input, states, score_ta, y_predicted_ta,
                                        y_ta, x_predicted_ta, urine_output_predicted_ta])
    score_ta = returned[-5]
    y_predicted_ta = returned[-4]
    x_predicted_ta = returned[-2]
    urine_output_predicted_ta = returned[-1]
    ...
    scores = score_ta.stack()  # [num_steps, batchSize]
    scores = tf.transpose(scores, [1, 0])  # [batchSize, num_steps]
    ...
    urine_output_predicted = urine_output_predicted_ta.stack()  # [num_steps, batchSize]
    urine_output_predicted = tf.transpose(urine_output_predicted, [1, 0])  # [batchSize, num_steps]
    mask_binary = tf.cast(mask > 0, tf.float32)
    objective_cost = tf.reduce_mean(tf.reduce_sum(((urine_output - urine_output_predicted) ** 2) * mask,
                                                  reduction_indices=[1]))
    mask_binary = tf.slice(mask_binary, [0, 1], [-1, -1])
    mask_binary = tf.expand_dims(mask_binary, -1)
    mask_binary = tf.tile(mask_binary, [1, 1, numFeatures])
    regularizer = ((x_predicted - x_shifted) ** 2) * mask_binary
    ...
    return {
        ...
        'y': y,
        'weight': weight,
        'urine_output': urine_output_placeholder,
...
def trainGraph(g, sess, train, test, epochs=10, batchSize=batchSize, learningRate=1e-4, momentum=0.9, optimizer='adam', regularizer_weight=0.05):
    ...
    while current_epoch < epochs:
        ...
        weight = batch[0][:, 0, -2]
        urine_output = batch[0][:, :, -1]
        feed = {g['x']: batch[0][:, :, :-2],
                g['y']: batch[1],
                g['seqlen']: batch[2],
                g['mask']: batch[3],
                g['weight']: weight,
                g['urine_output']: urine_output,
                g['keepProb']: 0.5,
        ...
        if tr.epochs > current_epoch:
            ...
            while te.epochs <= test_epoch:
                ...
                weight = batch[0][:, 0, -2]
                urine_output = batch[0][:, :, -1]
                feed = {g['x']: batch[0][:, :, :-2], g['y']: batch[1],
                        g['seqlen']: batch[2], g['mask']: batch[3],
                        g['keepProb']: 1, g['weight']: weight, g['urine_output']: urine_output}
                size = len(batch[0])
